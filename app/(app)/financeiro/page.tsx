...
export default function FinanceiroPage() {
  ...
  const [typeNew, setTypeNew] = useState<'payable' | 'receivable'>('payable')
  ...
  function openNew(type: 'payable' | 'receivable') { setTypeNew(type); setShowNew(true) }
    ...
    <div className="space-y-5 max-w-5xl">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {(filters.type || filters.status || filters.month) && (
          <Button variant="ghost" size="sm" onClick={() => setFilters({ type: '', status: '', month: '' })}>
            Limpar filtros
          </Button>
        )}
      </div>
        ...
        {loading ? (
          <div className="divide-y">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="h-16 animate-pulse bg-gray-100 m-2 rounded-lg" />
            ))}
          </div>
    ...
    <div className="px-5 py-3.5 flex items-center gap-3 hover:bg-gray-50/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-400">
          {(t.obra as { name?: string } | undefined)?.name ?? 'Sem obra'} · {t.category ?? 'Sem categoria'} · Vence {formatDate(t.due_date)}
        </p>
      </div>
...
function NewTransactionModal({
  ...
  open: boolean
  onClose: () => void
  onCreate: (payload: Partial<Transaction>) => Promise<unknown>
