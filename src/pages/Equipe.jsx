...
import { Button, Card, Badge, Modal, Input, Select, EmptyState } from '../components/ui/index.jsx'
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      ...
      <Card>
        ...
        {loading ? (
          <div className="divide-y">{[...Array(3)].map((_, i) => <div key={i} className="h-16 animate-pulse m-3 bg-gray-100 rounded-lg" />)}</div>
        ) : members.length === 0 ? (
            ...
            {invites.map(inv => (
              <div key={inv.id} className="px-5 py-3.5 flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400">Expira em {new Date(inv.expires_at).toLocaleDateString('pt-BR')}</p>
                </div>
      ...
      <EditRoleModal open={!!editMember} member={editMember} onClose={() => setEditMember(null)} onSave={updateMemberRole} />
    </div>
  )
}
...
function InviteModal({ open, onClose, onInvite }) {
  ...
  async function handleSubmit(e) {
    ...
    setSaving(false)
    if (!error) { setSuccess(true); setTimeout(() => { setSuccess(false); onClose(); setForm({ email: '', role: 'operational' }) }, 2000) }
  }
    ...
    <Modal open={open} onClose={onClose} title="Convidar membro">
      {success ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="Email do colaborador" type="email" value={form.email}
            onChange={e => setForm(p => ({...p, email: e.target.value}))}
            placeholder="colaborador@empresa.com" required />
          <Select label="Cargo" value={form.role} onChange={e => setForm(p => ({...p, role: e.target.value}))}>
...
function EditRoleModal({ open, member, onClose, onSave }) {
  const [role, setRole] = useState(member?.role ?? 'operational')
  const [saving, setSaving] = useState(false)
