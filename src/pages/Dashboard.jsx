...
import { StatCard, Card, Badge, ProgressBar, currency, formatDate, daysUntil } from '../components/ui/index.jsx'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {can('canViewFinancial') && (
          <Card>
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <Link to="/financeiro" className="text-xs text-[#2C5530] font-medium hover:underline">Ver todos</Link>
            </div>
            <div className="divide-y divide-gray-50">
              {stats?.upcomingTransactions?.length === 0 && (
                <p className="px-5 py-8 text-sm text-center text-gray-400">Nenhum vencimento próximo 🎉</p>
              )}
              {stats?.upcomingTransactions?.map(t => {
                const days = daysUntil(t.due_date)
                return (
                  <div key={t.id} className="px-5 py-3 flex items-center gap-3">
                    <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${t.type === 'receivable' ? 'bg-[#4A9B5C]' : 'bg-red-400'}`} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-gray-900 truncate">{t.description}</p>
                      <p className="text-xs text-gray-400">{t.obra?.name ?? 'Sem obra'} · {formatDate(t.due_date)}</p>
                    </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <QuickLink to="/obras/new" icon={Building2} label="Nova obra" desc="Cadastrar projeto" />
            {can('canViewFinancial') && <QuickLink to="/financeiro/new" icon={Wallet} label="Lançamento" desc="Registrar transação" />}
            <QuickLink to="/obras" icon={Building2} label="Ver obras" desc={`${stats?.activeObras ?? 0} em andamento`} />
            {can('canManageUsers') && <QuickLink to="/equipe" icon={Wallet} label="Equipe" desc="Gerenciar usuários" />}
          </div>
        </div>
      </div>
    </div>
...
function QuickLink({ to, icon: Icon, label, desc }) {
  return (
    <Link to={to} className="group block bg-white rounded-xl border border-[#2C5530]/10 p-4 hover:border-[#4A9B5C]/40 hover:shadow-sm transition-all">
